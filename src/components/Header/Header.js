...
export default function Header({}) {
  ...
  const isCurrentPath = (matchPatterns) => {
    return matchPatterns.some(pattern => matchPath({ path: pattern, end:true }, currentPath));
  }

  const isHeaderSticky = () => {
    const notStickyUrls = ['/fundraisings/:id/story'];
    return !isCurrentPath(notStickyUrls);
  }

  return (
    <S.Header id='header' $sticky={isHeaderSticky()}>
      <S.HeaderInner>
        <S.MainLogo as='h1'>
      ...
      <S.SideMenuOverlay className={isSideMenuOpen ? 'open' : 'close'} onClick={closeSideMenu}>
